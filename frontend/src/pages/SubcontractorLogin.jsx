...
import { loginSubcontractor, clearError } from '../store/slices/authSlice';
...
const SubcontractorLogin = () => {
    ...
    const handleLogin = async (e) => {
        e.preventDefault();
        if (email && password) {
            dispatch(loginSubcontractor({ email, password }));
        }
    };
        ...
        >
            <div className="glass-panel w-full max-w-md p-8 rounded-3xl animate-[fadeIn_0.5s_ease-out_forwards]">
                <div className="text-center mb-8">
                    ...
                    <p className="text-emerald-400 mt-2 font-bold text-sm tracking-wider uppercase">Subunternehmer Portal</p>
                </div>
                ...
                <form onSubmit={handleLogin} className="flex flex-col gap-5">
                    ...
                    <div className="flex flex-col gap-2">
                        <label className="text-sm text-slate-300 font-medium px-1">Subunternehmer E-Mail</label>
                    ...
                    <button
                        type="submit"
                        disabled={isLoading}
                        className="mt-4 bg-emerald-600 hover:bg-emerald-500 text-white font-semibold py-3.5 px-4 rounded-xl transition-all shadow-[0_0_20px_rgba(16,185,129,0.3)] hover:shadow-[0_0_25px_rgba(16,185,129,0.5)] flex items-center justify-center gap-2 group disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        {isLoading ? (
